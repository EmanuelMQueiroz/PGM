## Projected Gradient Method with PG4 ##

function PG4(x, f, ∇f, projection, σ, min_step, β_start)
    ierror = 0
    β = β_start
    j = 0
    ...
    return (β, zkj, ierror, evalf, projeval)
end

function method4(x, f, ∇f, ε, max_iter, PG4, projection)
    ...
    if norm(x - projection(x - ∇f(x))) < ε
        ...
        return (x, f(x), info, et, ierror, seqx, sum(evalf_β), sum(projevals))
    end
    ...
    while true
        ...
        fx = f(x)
        gradnorm = norm(∇fx)
        (β, zkj, ierror, evalf, projeval) = PG4(x, f, ∇f, projection, σ, min_step, β_start)
